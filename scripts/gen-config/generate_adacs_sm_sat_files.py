# Usage: python3 generate_adacs_sm_sat_files.py count /path/to/seed.tle /path/to/seed.dat /path/to/dst/
# Parameters:
#  count: number of satellites in the constellation
#  /path/to/seed.tle: 3-line TLE (header, line one, line two) with seed NORAD ID
#  /path/to/seed.dat: adacs-sm-sat.dat seed file to be copied with new file name
...
if len(sys.argv)==5:
  ...
  print(\
   'Usage: '\
   'python3 generate_adacs_sm_sat_files.py '\
...
for i in range(0,cnt):
  ...
  # generate file name
  filename = 'adacs-sm-sat-'+id_str+'.dat'
