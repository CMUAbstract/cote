...
# Output:
#  A CSV file of binned altitude-deadline pairs
...
if len(sys.argv)==3:
  ...
  with open(dst+'binned.csv',mode='w',newline='') as csv_o:
    ...
    csvwriter.writerow(['alt-km','deadline-s','alt-stdev-km','deadline-stdev-s'])
else:
...
with open(src,newline='') as csv_i:
  ...
  for row in csvreader:
    ...
    if alt_bin not in deadline_sums:
      deadline_sums[alt_bin] = 0.0
    if alt_bin not in counts:
      counts[alt_bin] = 0
    altitude_sums[alt_bin] += altitude
    deadline_sums[alt_bin] += deadline
    counts[alt_bin] += 1
...
with open(dst+'binned.csv',mode='a',newline='') as csv_o:
  csvwriter = csv.writer(csv_o)
  for key in keys:
    csvwriter.writerow(['{:05.6f}'.format(altitude_sums[key]/counts[key]),'{:03.6f}'.format(deadline_sums[key]/counts[key])])
