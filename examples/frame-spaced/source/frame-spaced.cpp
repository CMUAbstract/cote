// frame-spaced.cpp
//
// Simulates a constellation of satellites with camera sensors that collectively
// capture observations at the ground track frame rate and attempt to downlink
// all data to ground stations.
//
    ...
    if(distanceKm>=satId2ThresholdKm[LEAD_SAT_ID]) {
      ...
      if(frameCount%satellites.size()==0) {
        ...
        for(size_t i=0; i<satellites.size(); i++) {
          ...
          satId2ThresholdKm[satellites.at(i).getID()] =
           threshCoeff*cote::util::calcAltitudeKm(
            satellites.at(i).getECIPosn()
           );
        }
      } else { // a frame has advanced, register with lead sat w/o getting data
        satId2Sensor[LEAD_SAT_ID]->setPrevSensePosnDateTime(currPosn,dateTime);
      }
    }
