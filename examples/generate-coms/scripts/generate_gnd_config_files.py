# Usage: python3 generate_gnd_config_files.py lat lon n id0 /path/to/dst/
# Input:
...
if len(sys.argv)==6:
  ...
  lon = float(sys.argv[2])%360.0 # start of trick
  n = int(sys.argv[3])
  id0 = int(sys.argv[4])
  dst = sys.argv[5]
  if dst[-1] != '/':
  ...
  print("Usage: python3 generate_gnd_config_files.py lat lon n id0 /path/to/dst/")
  exit()
...
for i in range(0,len(lons)):
  lons[i] = ((lons[i]+180.0)%360.0)-180.0 # end of trick
