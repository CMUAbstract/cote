#!/usr/bin/env python3
# Usage: python3 plot_ground_tracks_split.py /path/to/logs/ /path/to/plot.ext
# Input:
...
if len(sys.argv)==3:
  ...
  print('Usage: python3 plot_ground_tracks_split.py /path/to/logs/ /path/to/plot.ext')
  exit()
...
for id in ground_tracks:
  ...
  plt.savefig(dst[:-4]+'-'+str(id).zfill(5)+dst[-4:])
  plt.cla()
  plt.clf()
  plt.close()
