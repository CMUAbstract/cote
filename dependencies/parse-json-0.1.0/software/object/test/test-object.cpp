...
int main(int argc, char** argv) {
  ...
  if(argc!=3) {
    ...
    while(it!=std::filesystem::end(it)) {
      ...
      if(pathStr.substr(pathStr.size()-12,12)=="input00.json") {
        ...
        input00 = new json::Object(fileStr);
  ...
  // Perform tests
  bool pass;
  std::map<std::string,json::Value*> objectValue = input00->getValue();
  std::cout << "Object entry \"0\" is true?"
            << std::endl;
  pass = (dynamic_cast<json::Bool*>(objectValue["0"])->getValue()==true);
  std::cout << "  " << (pass ? "PASS" : "FAIL")
            << std::endl;
  std::cout << "Object entry \"1\" is null?"
            << std::endl;
  pass = (dynamic_cast<json::Nul*>(objectValue["1"])->getValue()==nullptr);
  std::cout << "  " << (pass ? "PASS" : "FAIL")
            << std::endl;
  std::cout << "Object entry \"2\" is 0.59?"
            << std::endl;
  pass = (dynamic_cast<json::Number*>(objectValue["2"])->getValue()==0.59);
  std::cout << "  " << (pass ? "PASS" : "FAIL")
            << std::endl;
  std::cout << "Object entry \"3\" is \"Hello world!\"?"
            << std::endl;
  pass = (
   dynamic_cast<json::String*>(objectValue["3"])->getValue()=="Hello world!"
  );
  std::cout << "  " << (pass ? "PASS" : "FAIL")
            << std::endl;
  // Write out files
